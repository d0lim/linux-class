							   ...
							   struct pxt4_group_desc *bg)
{
	return le32_to_cpu(bg->bg_block_bitmap_lo) |
		   (PXT4_DESC_SIZE(sb) >= PXT4_MIN_DESC_SIZE_64BIT ? (pxt4_fsblk_t)le32_to_cpu(bg->bg_block_bitmap_hi) << 32 : 0);
}
							   ...
							   struct pxt4_group_desc *bg)
{
	return le32_to_cpu(bg->bg_inode_bitmap_lo) |
		   (PXT4_DESC_SIZE(sb) >= PXT4_MIN_DESC_SIZE_64BIT ? (pxt4_fsblk_t)le32_to_cpu(bg->bg_inode_bitmap_hi) << 32 : 0);
}
							  ...
							  struct pxt4_group_desc *bg)
{
	return le32_to_cpu(bg->bg_inode_table_lo) |
		   (PXT4_DESC_SIZE(sb) >= PXT4_MIN_DESC_SIZE_64BIT ? (pxt4_fsblk_t)le32_to_cpu(bg->bg_inode_table_hi) << 32 : 0);
}
							   ...
							   struct pxt4_group_desc *bg)
{
	return le16_to_cpu(bg->bg_free_blocks_count_lo) |
		   (PXT4_DESC_SIZE(sb) >= PXT4_MIN_DESC_SIZE_64BIT ? (__u32)le16_to_cpu(bg->bg_free_blocks_count_hi) << 16 : 0);
}
							 ...
							 struct pxt4_group_desc *bg)
{
	return le16_to_cpu(bg->bg_free_inodes_count_lo) |
		   (PXT4_DESC_SIZE(sb) >= PXT4_MIN_DESC_SIZE_64BIT ? (__u32)le16_to_cpu(bg->bg_free_inodes_count_hi) << 16 : 0);
}
						   ...
						   struct pxt4_group_desc *bg)
{
	return le16_to_cpu(bg->bg_used_dirs_count_lo) |
		   (PXT4_DESC_SIZE(sb) >= PXT4_MIN_DESC_SIZE_64BIT ? (__u32)le16_to_cpu(bg->bg_used_dirs_count_hi) << 16 : 0);
}
							   ...
							   struct pxt4_group_desc *bg)
{
	return le16_to_cpu(bg->bg_itable_unused_lo) |
		   (PXT4_DESC_SIZE(sb) >= PXT4_MIN_DESC_SIZE_64BIT ? (__u32)le16_to_cpu(bg->bg_itable_unused_hi) << 16 : 0);
}
...
{
	return system_state == SYSTEM_HALT || system_state == SYSTEM_POWER_OFF || system_state == SYSTEM_RESTART;
}
...
static const struct mount_opts
{
	...
	{Opt_journal_checksum, PXT4_MOUNT_JOURNAL_CHECKSUM,
	 MOPT_PXT4_ONLY | MOPT_SET | MOPT_EXPLICIT},
	{Opt_journal_async_commit, (PXT4_MOUNT_JOURNAL_ASYNC_COMMIT | PXT4_MOUNT_JOURNAL_CHECKSUM),
	 MOPT_PXT4_ONLY | MOPT_SET | MOPT_EXPLICIT},
	...
	{Opt_prjquota, PXT4_MOUNT_QUOTA | PXT4_MOUNT_PRJQUOTA,
	 MOPT_SET | MOPT_Q},
	{Opt_noquota, (PXT4_MOUNT_QUOTA | PXT4_MOUNT_USRQUOTA | PXT4_MOUNT_GRPQUOTA | PXT4_MOUNT_PRJQUOTA),
	 MOPT_CLEAR | MOPT_Q},
	...
	for (i = 0; i < sbi->s_groups_count; i++)
	{
		...
		if (!pxt4_group_desc_csum_verify(sb, i, gdp))
		{
			pxt4_msg(sb, KERN_ERR, "pxt4_check_descriptors: "
					 ...
					 i, le16_to_cpu(pxt4_group_desc_csum(sb, i, gdp)), le16_to_cpu(gdp->bg_checksum));
			if (!sb_rdonly(sb))
			{
		...
		list_for_each_safe(pos, n, &eli->li_request_list)
		{
			...
			if (!progress)
			{
				elr->lr_next_sched = jiffies +
									 (prandom_u32() % (PXT4_DEF_LI_MAX_START_DELAY * HZ));
			}
			if (time_before(elr->lr_next_sched, next_wakeup))
				next_wakeup = elr->lr_next_sched;
		}
...
static int pxt4_fill_super(struct super_block *sb, void *data, int silent)
{
	...
	case 0:
		 ...
		 */
		if (jbd3_journal_check_available_features(sbi->s_journal, 0, 0, JBD3_FEATURE_INCOMPAT_REVOKE))
		{
			set_opt(sb, ORDERED_DATA);
							 ...
							 unsigned long journal_devnum)
{
	...
	if (!err)
	{
		...
		if (save)
			memcpy(save, ((char *)es) + PXT4_S_ERR_START, PXT4_S_ERR_LEN);
		...
		es->s_kbytes_written =
			cpu_to_le64(PXT4_SB(sb)->s_kbytes_written +
						((part_stat_read(sb->s_bdev->bd_part,
										 sectors[STAT_WRITE]) -
						  PXT4_SB(sb)->s_sectors_written_start) >>
						 1));
	else
		es->s_kbytes_written =
			cpu_to_le64(PXT4_SB(sb)->s_kbytes_written);
							   ...
							   kprojid_t projid, struct kstatfs *buf)
{
	...
	if (limit && buf->f_blocks > limit)
	{
		curblock = (dquot->dq_dqb.dqb_curspace +
					dquot->dq_dqb.dqb_rsvspace) >>
				   sb->s_blocksize_bits;
		buf->f_blocks = limit;
		buf->f_bfree = buf->f_bavail =
			(buf->f_blocks > curblock) ? (buf->f_blocks - curblock) : 0;
	}
	...
	if (limit && buf->f_files > limit)
	{
		buf->f_files = limit;
		buf->f_ffree =
			(buf->f_files > dquot->dq_dqb.dqb_curinodes) ? (buf->f_files - dquot->dq_dqb.dqb_curinodes) : 0;
	}
